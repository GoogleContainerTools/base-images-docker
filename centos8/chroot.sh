...
yum -y -q --releasever=${1} install yum centos-linux-release
yum install -q -y bind-utils     bash     yum     vim-minimal     centos-linux-release     less     iputils     iproute     systemd     rootfiles     tar     passwd     yum-utils     yum-plugin-ovl    hostname  which
